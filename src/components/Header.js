...
export default function Header() {
  ...
  if (navigation.categories.length === 0) {
    entry?.nav_items?.forEach(item => {
      if (item.mega_menu.length > 0) {
        let obj = {
          name: item.label,
          megamenu: {
            groups: []
        ...
        item.mega_menu.forEach(mm => {
          let group_items = [];
          mm.group_name[0].group_items.forEach(gi => {
            group_items.push(
              {
                name: gi.item,
                href: '#'
          ...
          obj.megamenu.groups.push(
            {
              name: mm.group_name[0].group_name,
              items: group_items
            }
          )
        })
        navigation.categories.push(obj);
      }
      else {
        ...
        <nav aria-label="Top">
          ...
          <div className="bg-gray-900">
            <div className="mx-auto flex h-10 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
              ...
              <p className="flex-1 text-center text-sm font-medium text-white lg:flex-none" {...entry.$?.headline}>
                {entry.headline}
              </p>
                                      ...
                                      <div className="mx-auto max-w-7xl px-8">
                                        <div className=" items-start gap-x-8 gap-y-10 pb-12 pt-10">
                                          <div className="grid grid-cols-4 gap-x-8 gap-y-10">
                                            {category.megamenu.groups.map((group) => (
                                              <div>
                                                <p
